    ...
    */
    ...
    void Update()
    {
        ...
        //if (false)
        {
            ...
            Debug.Log("\n SPM: Inside the changing Video section");
            Debug.Log("\n SPM: the current time before started playing = " + Time.realtimeSinceStartup);


            //m_URI = "file:///C:/Users/spauldsnl/Documents/decoding_videos/viking_texas_Ionly/" + fID.ToString() + ".mp4"; //D:/decoding_videos/of_Ionly/"
            m_URI = "file:///sdcard/viking_play/" + fID.ToString() + ".mp4";
            double start_time_t = Time.realtimeSinceStartup;
            Debug.Log("\n SPM: Before Calling Setup(), fileID=" + m_URI);
            Setup(m_URI, m_VideoIndex, m_AudioIndex);
            Debug.Log("\n  SPM: setup time taken = " + (Time.realtimeSinceStartup - start_time_t) * 1000);
            Debug.Log("\n SPM: Before Calling Play(), current counter = " + counter);
            Play();
            Debug.Log(" \n SPM: 1/fps = " + Time.unscaledDeltaTime * 1000);
            Debug.Log("\n SPM: the total time = " + (Time.realtimeSinceStartup - start_time) * 1000);
            if (fID >= 100) fID = 0;
            change = 1;
    ...
    void LateUpdate()
    {
        ...
        {
            double start_resize = Time.realtimeSinceStartup;
            if (change == 1)
            {
            ...
            //Debug.Log("\n Resize = " + (int)sz.x + "," + (int)sz.y);
            //Debug.Log("\n The time taken by Resize = " + (Time.realtimeSinceStartup - start_resize));
            ...
            {
                double start_btext = Time.realtimeSinceStartup;
                m_Pipeline.BlitTexture(m_Texture.GetNativeTexturePtr(), m_Texture.width, m_Texture.height);
               // Debug.Log("\n The time taken by textureupdate = " + (Time.realtimeSinceStartup - start_btext));
                /*
                if (change == 1)
                {
